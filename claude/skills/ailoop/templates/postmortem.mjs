#!/usr/bin/env node
// postmortem.mjs — render the campaign's journal as a self-contained HTML
// post-mortem: stat tiles, a Gantt timeline (one lane per ticket, dependency
...
// those are priced at the model's output rate. Coordinator-session overhead
// is not visible from inside the run and is excluded — the page says so.
...
const journal = fs.readFileSync(path.join(RUN, 'journal.jsonl'), 'utf8')
  .split('\n').filter(Boolean).map(l => JSON.parse(l));

// $/MTok output rate, checked 2026-07 — refresh when models rotate
const OUTPUT_PRICE = { opus: 25, sonnet: 15, haiku: 5 };

const t0 = new Date(journal[0].ts).getTime();
const t1 = new Date(journal.at(-1).ts).getTime();
const x = ts => (new Date(ts).getTime() - t0) / (t1 - t0);

// ---- derive per-ticket lifecycle from the journal ----
const tickets = {};
const forTicket = id => tickets[id] ||= { id, spans: [], verifies: [], attempts: 0 };
...
for (const t of Object.values(tickets)) for (const s of t.spans) {
  if (!s.end) { s.end = journal.at(-1).ts; s.outcome = 'open'; }
}

const rows = Object.values(tickets)
  .map(t => ({ ...t, meta: backlog.tickets.find(b => b.id === t.id) || {} }))
  .filter(t => t.spans.length)
  .sort((a, b) => a.spans[0].start < b.spans[0].start ? -1 : 1);

const estCost = t => {
  const tokens = t.closeData?.workerTokens ?? t.spans.reduce((s, sp) => s + (sp.data?.workerTokens || 0), 0);
...
console.log(`post-mortem written to ${OUT} (${rows.length} tickets, ${journal.length} journal events embedded)`);
